...
$(document).on("keyup", ".QTY", function () {
  ...
  if (parseInt(originalqty.value) <= parseInt(qty.value)) {
    alert(
      "The quantity that you put is greater than the availabe quantity of the product."
    );
  ...
  document.getElementById("Osubtot" + productid).value =
    document.getElementById("TOT" + productid).value;

  var table = document.getElementById("table");
  var items = table.getElementsByTagName("output");

  var sum = 0;
  for (var i = 0; i < items.length; i++) sum += parseFloat(items[i].value);
  ...
  $.ajax({
    ...
    type: "POST",
    url:
      "cart/controller.php?action=edit&QTY" +
      productid +
      "=" +
      qty.value +
      "&TOT" +
...
$(document).on("change", ".QTY", function () {
  ...
  if (parseInt(originalqty.value) <= parseInt(qty.value)) {
    alert(
      "The quantity that you put is greater that the availabe quantity of the product."
    );
    document.getElementById("QTY" + productid).value = originalqty.value;
